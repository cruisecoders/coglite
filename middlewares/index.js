...
exports = module.exports = function (app, express, root) {
    ...
    if (config.get('server.enableStatic')){
    	//To server different html based on hostname (Reseller Changes)
    	// app.use(function (req, res, next) {
        //     if (path.extname(req.path).length > 0) {
        //             //normal static file request
        //             next();
        //     }else {//request for index.html
        //             	//If local then provide dev file
    	...
    	//Server static files
        var directories=config.get('server.static.directories');
        console.log(directories);
        for (x of directories) {
   console.log(directories[x]);
}
